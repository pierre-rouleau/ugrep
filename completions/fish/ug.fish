...
complete -c ug -s t -l file-type -xa 'actionscript ada adoc asm asp aspx autoconf automake awk Awk basic batch bison c c3 c++ clojure coffeescript cpp crystal csharp css csv dart Dart delphi elisp elixir erlang faust fennel fsharp fortran gif Gif go groovy gsp haskell html jade janet java jpeg Jpeg js json jsp julia kotlin less lex lisp lua m4 make markdown matlab nim nix node Node objc objc++ ocaml org parrot pascal pdf Pdf perl Perl php Php png Png protobuf prolog python Python r racket rpm Rpm rst rtf Rtf ruby Ruby rust scala scheme seed7 shell Shell smalltalk sql svg swift tcl tex text tiff Tiff tt typescript vala verilog vhdl vim xml Xml yacc yaml zig ' -d 'Search only files associated with TYPES, a comma-separated list of file types'
